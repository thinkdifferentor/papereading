...
## [34 Rethinking Federated Learning with Domain Shift: A Prototype View](./domainadaptation/Rethinking%20Federated%20Learning%20with%20Domain%20Shift%20A%20Prototype%20View.pdf)
- Huang W, Ye M, Shi Z, et al./2023/CVPR/22
- However, prevalent solutions of federated learning mainly focus on all private data sampled from the same domain. *An important challenge is that when distributed data are derived from diverse domains.* The private model presents degenerative performance on other domains (with domain shift). (faces a slow convergence speed and achieves limited performance improvement)
- The core idea is to construct **cluster prototypes** and **unbiased prototypes**, providing **fruitful domain knowledge and a fair convergent target**. On the one hand, we pull the sample embedding closer to cluster prototypes belonging to the same semantics than cluster prototypes from distinct classes. On the other hand, we introduce consistency regularization to align the local instance with the respective unbiased prototype.
- Specifically, the private model fails to provide discrimination on other domains because it **overfits** local domain distribution. The
aforementioned methods mainly regulate the private model via global knowledge.  Therefore, these algorithms share a common weakness: *the global information is insufficient to describe diverse domain knowledge*, which is magnified under the domain shift and thus hinders the improvement of generalizability.
- Given the enormous participant scale in federated learning, it is not efficient and feasible to maintain all prototypes. For global prototypes, averaging operation **weakens the domain diversity** and yield biased to **the dominant domain** due to the unknown of private domains proportion, which results in disadvantageous performance on minority domains.
- ![FPL_1](./images/FPL_1.png)
- ![FPL_2](./images/FPL_2.png)
...
## [23 Robust and generalizable visual representation learning via random convolutions](./domaingeneralization/Robust%20and%20generalizable%20visual%20representation%20learning%20via%20random%20convolutions.pdf)
- Xu Z, Liu D, Yang J, et al./2021/ICLR/123
